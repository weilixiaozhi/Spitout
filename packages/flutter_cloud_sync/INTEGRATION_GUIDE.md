...
本文档说明如何将新的 `flutter_cloud_sync` 包集成到消费方主应用中。
  ...
  switch (config.type) {
    ...
    case CloudBackendType.supabase:
      ...
      await provider.initialize({
        ...
        'bucket': 'my-app-data', // 自定义 bucket 名称
      });
      return provider;
    ...
    case CloudBackendType.webdav:
      ...
      await provider.initialize({
        ...
        'remotePath': config.webdavRemotePath ?? '/my-app/',
      });
      return provider;
  }
}
...
class CloudSyncServiceImpl implements SyncService {
  ...
  /// 转换 flutter_cloud_sync 的 SyncStatus 到消费方应用的 SyncStatus
