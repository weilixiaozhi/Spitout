...
void main() {
  ...
  setUp(() {
    ...
    when(() => repo.transactionsWithCategoryAll(ledgerId: any(named: 'ledgerId')))
  ...
  testWidgets(
      '手指向左滑切下月：selectedMonth 仅加 1，不会被重复偏移到离谱年份（bug1 回归）',
      (tester) async {
    ...
    expect(after, DateTime(2026, 8, 1),
        reason: '滑动一次应仅切一个月，修复前会因重入循环偏移到离谱年份');
  });
  ...
  testWidgets('手指向右滑切上月：selectedMonth 减 1', (tester) async {
    ...
    expect(containerOf(tester).read(selectedMonthProvider), DateTime(2026, 6, 1));
  });

  testWidgets('拖动未达 80% 阈值回弹：selectedMonth 不变（不误切月）',
      (tester) async {
    await tester.pumpWidget(buildApp(initialMonth: DateTime(2026, 7, 1)));
    await prime(tester);
  ...
  testWidgets(
      '连续向左滑两次：selectedMonth 递增两次，每次仅加 1（防重入 + 串行切月）',
      (tester) async {
    await tester.pumpWidget(buildApp(initialMonth: DateTime(2026, 7, 1)));
    await prime(tester);
    ...
    await tester.fling(find.byType(PageView), const Offset(-700, 0), 4000);
    await settleSwipe(tester);
    expect(containerOf(tester).read(selectedMonthProvider), DateTime(2026, 8, 1));

    // 第二次 → 2026-09。
    await tester.fling(find.byType(PageView), const Offset(-700, 0), 4000);
    await settleSwipe(tester);
    expect(containerOf(tester).read(selectedMonthProvider), DateTime(2026, 9, 1));
  });

  testWidgets('回到当月按钮：点击后 selectedMonth 回到当前自然月（bug3 回归）',
      (tester) async {
    ...
    expect(backToCurrentFinder, findsOneWidget,
        reason: '非当月时应展示「回到当月」按钮');

    await tester.tap(backToCurrentFinder);
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 50));

    expect(containerOf(tester).read(selectedMonthProvider), currentMonth,
        reason: '点击回到当月后 selectedMonth 应等于当前自然月');
  });

  testWidgets('空数据状态：stream 发射空列表时显示空状态占位（AppEmpty）',
      (tester) async {
    registerTxsStream(() => Stream<List<_TxItem>>.value(const []));
    await tester.pumpWidget(buildApp());
    await prime(tester);
  ...
  testWidgets('MonthPickerSheet 初始定位与 selectedMonth 一致（bug2 回归）',
    ...
    expect(find.text('07月 · 2026年'), findsWidgets,
        reason: '日期头应回显选中月 07 月，验证 picker 初始定位与 selectedMonth 一致');
  });

  testWidgets('selectedMonth 不会出现负数/远古年份（bug4 极端值回归）',
      (tester) async {
    await tester.pumpWidget(buildApp(initialMonth: DateTime(2026, 7, 1)));
    await prime(tester);
    ...
    expect(after.year, greaterThan(2000),
        reason: '滑动切月不应产生远古/负数年份');
  });
  ...
  group('汇总卡今日/本周行（卡片高度固定）', () {
    testWidgets('非当月：今日/本周行常驻不隐藏，金额以 "-" 占位', (tester) async {
      ...
      expect(find.text(l10n.homeTodayExpense), findsOneWidget,
          reason: '非当月今日标签常驻');
      expect(find.text(l10n.homeWeekExpense), findsOneWidget,
          reason: '非当月本周标签常驻');
    ...
    testWidgets('当月：今日/本周行显示真实金额（0 直接显示 0，无 + 号）', (tester) async {
      final now = DateTime.now();
      await tester
          .pumpWidget(buildApp(initialMonth: DateTime(now.year, now.month, 1)));
      await prime(tester);

      final l10n = AppLocalizations.of(tester.element(find.byType(HomePage)));
      // provider 固定 0.0 → 金额由 AmountText 渲染,与卡片主金额同源(均走
      // formatMoneyWithCurrency),避免脆断。下面分别断言标签与金额位。
      final zero = formatMoneyWithCurrency(0.0, currencyCode: testLedger.currency);
      expect(find.text(l10n.homeTodayExpense), findsOneWidget,
          reason: '当月今日标签常驻');
      expect(find.text(l10n.homeWeekExpense), findsOneWidget,
          reason: '当月本周标签常驻');
  ...
  group('首页头部布局（Figma 53:6）', () {
    testWidgets('首页首行由 PrimaryHeader 渲染且使用全局默认留白（上/下 10、左/右 14）', (tester) async {
      await tester.pumpWidget(buildApp());
      await prime(tester);
      ...
      expect(header.onTitleTap, isNotNull,
    ...
    testWidgets('首页头部不再渲染日历本按钮（入口已迁至底部导航栏）', (tester) async {
      ...
      expect(find.text(l10n.calendarTitle), findsNothing,
    ...
    testWidgets('轻扫提示行位于日期头与汇总卡之间，左缘距 14', (tester) async {
      ...
      expect(tester.getCenter(hintFinder).dy,
    ...
    testWidgets('汇总卡内边距为 all(20)，账本徽章以 tab 挂在卡片右缘', (tester) async {
      ...
      expect(tester.getCenter(badgeFinder).dx, greaterThan(screenWidth / 2),
          reason: '账本徽章应贴在卡片右缘（右半屏）');
      final dyDiff = (tester.getCenter(badgeFinder).dy -
    ...
    testWidgets('非当月：「回到当月」位于日期组件右侧、与日期头同行', (tester) async {
      ...
      final dateFinder = find.byWidgetPredicate((w) =>
          w is Text && (w.data ?? '').contains('${nonCurrent.year}年'));
      ...
      // 且位于日期组件右侧
      expect(tester.getCenter(backFinder).dx,
  ...
  // ==================== 共享账本灰屏回归（交易流缓存） ====================
  testWidgets(
      '共享账本：账本信息变化触发 _MonthPage rebuild 时不重建交易流（灰屏回归）',
      (tester) async {
    ...
    registerTxsStream(() => repo.transactionsWithCategoryAll(ledgerId: 1));

    await tester.pumpWidget(buildApp(
      currentLedgerOverride:
          currentLedgerProvider.overrideWith((ref) => ledgerCtrl.stream),
    ));
    await prime(tester);

    // 首帧渲染后，交易流应至少被创建一次（initState 或首次 build）。
    final countAfterPrime = txStreamCallCount;
    expect(countAfterPrime, greaterThanOrEqualTo(1),
    ...
    expect(txStreamCallCount, countAfterPrime,
        reason: 'currentLedgerProvider 变化触发 _MonthPage rebuild 时不应重建交易流'
  ...
  testWidgets('共享账本带成员时首页渲染不崩溃（const {} 不可变 Map 写入崩溃回归）',
      (tester) async {
  ...
  Future<void> pullToRefresh(WidgetTester tester) async {
    ...
    for (var i = 1; i <= 4; i++) {
      ...
      ScrollUpdateNotification(metrics: metricsAt(-16.0 * i), context: ctx)
          .dispatch(ctx);
  ...
  group('首页下拉刷新：结果文案在指示器内展示，不再弹 toast（需求回归）', () {
    testWidgets('纯本地账本：刷新成功 → 指示器显示"已刷新本地账本数据与配置"且全局仅一处（无 toast）',
      ...
      expect(find.text(l10n.homeSyncing), findsNothing,
          reason: '刷新完成后指示器应切换为结果文案，不再显示"正在同步"');
      // 冲刷日志 2s 写入计时器与本功能 1s 收起计时器，避免 FakeAsync 报挂起定时器。
      await tester.pump(const Duration(seconds: 3));
    });

    testWidgets('云端账本：同步成功 → 指示器显示"已同步云端账本数据"且无 toast',
        (tester) async {
      final mock = _MockSyncService();
      when(() => mock.pullIncrementalWithHeal(ledgerId: any(named: 'ledgerId')))
          .thenAnswer((_) async => const PullOutcome(incremental: 1));
      await tester.pumpWidget(buildApp(
        extraOverrides: [
          syncServiceProvider.overrideWith((ref) => mock),
        ],
      ));
      await prime(tester);
    ...
    testWidgets('云端账本：同步失败但本地已刷新 → 指示器显示降级文案且无 toast',
        (tester) async {
      final mock = _MockSyncService();
      when(() => mock.pullIncrementalWithHeal(ledgerId: any(named: 'ledgerId')))
    ...
    testWidgets('结果文案停留约 1 秒后指示器收起（不再立即收起）',
        (tester) async {
      await tester.pumpWidget(buildApp(
        extraOverrides: [
          syncServiceProvider.overrideWith((ref) => LocalOnlySyncService()),
      ...
      expect(visibleIndicator.sizeFactor.value, closeTo(1.0, 0.1),
      ...
      expect(collapsedIndicator, isNotNull,
          reason: '结果停留 1 秒后指示器应平滑收起（sizeFactor≈0）；若不为 null 说明收起计时器未触发');
      expect(collapsedIndicator!.sizeFactor.value, closeTo(0.0, 0.1),
          reason: '结果停留 1 秒后指示器应平滑收起（sizeFactor≈0）');
      // 冲刷日志 2s 计时器，避免 FakeAsync 报挂起定时器。
      await tester.pump(const Duration(seconds: 3));
    });

    testWidgets('收起完成后结果文案归零：下次下拉即从"正在同步"开始，不残留上次结果',
        (tester) async {
      final mock = _MockSyncService();
      when(() => mock.pullIncrementalWithHeal(ledgerId: any(named: 'ledgerId')))
          .thenAnswer((_) async => const PullOutcome(incremental: 1));
      ...
      expect(find.text(l10n.homeSyncing), findsOneWidget,
          reason: '结果文案归零后，指示器应回到"正在同步"，确保下次下拉直接显示而非残留上次结果');
      expect(find.text(l10n.homePullCloudSuccess), findsNothing,
