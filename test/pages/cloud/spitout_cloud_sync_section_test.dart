...
void main() {
  ...
  testWidgets('未登录无凭证：卡片含登录按钮与同步状态详情', (tester) async {
    ...
    expect(
      find.descendant(of: _card(), matching: find.byIcon(AppIcons.login)),
      findsOneWidget,
    );
    ...
    expect(
      find.descendant(of: _card(), matching: find.text('同步状态')),
      findsOneWidget,
    );
    expect(
      find.descendant(of: _card(), matching: find.text('当前账本')),
      findsNothing,
    );
    expect(
      find.descendant(of: _card(), matching: find.text('未推送变更')),
      findsOneWidget,
    );
    expect(
  ...
  testWidgets('已登录：账号行只读展示账号,卡片含同步状态详情', (tester) async {
    ...
    expect(
      find.descendant(
        of: _card(),
        matching: find.byIcon(AppIcons.verifiedUser),
      ),
      findsOneWidget,
    );
  ...
  testWidgets('同步状态详情常驻:渲染本地/云端逐项计数行', (tester) async {
    ...
    expect(
      find.descendant(of: _card(), matching: find.text('全部账本')),
      findsOneWidget,
    );
    expect(
      find.descendant(of: _card(), matching: find.text('未推送变更')),
  ...
  testWidgets('选中第二本云账本 → 仍渲染「当前账本」组(不绑死 id 升序第一本)', (tester) async {
    final db = SpitoutDatabase.forTesting(NativeDatabase.memory());
    addTearDown(db.close);
    final tracker = ChangeTracker(db);
    final repo = LocalRepository(db, changeTracker: tracker);
    ...
    final ledgerB = await insertCloudLedger(
      db,
      name: 'Ledger B',
      syncId: 'ledger-b',
    );
  ...
  testWidgets('选中本地账本 → 隐藏「当前账本」组,「全部账本」组不降级', (tester) async {
    final db = SpitoutDatabase.forTesting(NativeDatabase.memory());
    addTearDown(db.close);
    final tracker = ChangeTracker(db);
    final repo = LocalRepository(db, changeTracker: tracker);
    ...
    expect(
      find.descendant(of: _card(), matching: find.text('暂无云端账本')),
      findsNothing,
  ...
  testWidgets('S4 账户级熔断:A 熔断、选中本地账本时状态行同样红字', (tester) async {
    final db = SpitoutDatabase.forTesting(NativeDatabase.memory());
    addTearDown(db.close);
    final tracker = ChangeTracker(db);
    final repo = LocalRepository(db, changeTracker: tracker);
    ...
    expect(
      find.descendant(of: _card(), matching: find.text('自动恢复失败，请从云端恢复')),
      findsOneWidget,
