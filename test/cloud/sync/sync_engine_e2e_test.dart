...
void main() {
  ...
  group('smoke', () {
    test('空 server → pull 返 0,不 prime LookupCache', () async {
      ...
      expect(
        provider.pullCalls.first.persistCursor,
        isFalse,
    ...
    test('cursor 在 SharedPreferences 持久化', () async {
      ...
      final keys = prefs.getKeys().where(
        (k) => k.startsWith('app_pull_cursor_'),
      );
  ...
  group('apply 远端 change', () {
    ...
    test('共享账本 Editor 拉取交易 分类必须走 override 不绑定到成员本地同 syncId 分类', () async {
      ...
      expect(tx.categoryId, isNull, reason: 'Editor 视角不应把 Owner 分类绑到成员本地正数 id');
  ...
  group('web 新建账本场景', () {
    test('server 推 ledger entity change → 本地 ledgers 表 insert', () async {
      ...
      provider.pushFakeChange(
        ...
        payload: {'ledgerName': 'My New Ledger', 'currency': 'USD'},
      );
  ...
  group('category seed 收编（父级作用域内唯一契约）', () {
    test('远端推「服装>鞋子」→ 按 level+parentId 收编到正确行，「购物>鞋子」不被误伤', () async {
      ...
      final clothingShoesId = await db
          .into(db.categories)
          .insert(
            CategoriesCompanion.insert(
              name: 'shoes',
      ...
      expect(rows, hasLength(4), reason: '收编应复用既有 seed 行，不应重复插入');

      // 「服装>鞋子」(id=clothingShoesId) 的 syncId 被补上
      final clothingShoes = rows.firstWhere((c) => c.id == clothingShoesId);
      expect(
        clothingShoes.syncId,
        'cat-shoes-clothing-remote',
        reason: '远端推的「服装>鞋子」应被收编到 parentId=clothingId 的行',
      );

      // 「购物>鞋子」(parentId=shoppingId) 的 syncId 仍为 null（未被误伤）
      final shoppingShoes = rows.firstWhere((c) => c.parentId == shoppingId);
      expect(shoppingShoes.syncId, isNull, reason: '同名不同父的「购物>鞋子」不应被错误收编');
    });
    ...
    test('远端推一级「服装」→ 收编到 parentId=null 的一级行，不命中二级「购物>服装」', () async {
      ...
      expect(subClothing.id, isNot(clothingId), reason: '收编目标行不应是二级行');
    });
  });
  ...
  group('push 路径', () {
    test('本地有 unpushed change → engine.push 推到 server', () async {
      ...
      final remaining = await changeTracker.getUnpushedChangesForLedger(
        ledgerId,
      );
  ...
  group('recordChanges=false:fullPull 不反向回流', () {
    test(
      ...
      () async {
        ...
        final changes = await changeTracker.getUnpushedChangesForLedger(
          ledgerId,
        );
        expect(
          changes,
  ...
  group('同步前状态检查 (getStatus)', () {
    test('isShared=true + myRole=editor → 不触发 fullPush', () async {
      ...
      expect(
        provider.writeCreateLedgerCalls,
        isEmpty,
  ...
  group('apply 各种 entity type', () {
    test('category insert', () async {
      provider.pushFakeChange(
        ...
        entitySyncId: 'cat-X',
        ledgerId: '',
        payload: {'name': 'NewCat', 'kind': 'expense', 'sortOrder': 0},
      );
    ...
    test('apply update 已存在的实体(按 syncId upsert,不重复 insert)', () async {
      ...
      provider.pushFakeChange(
        ...
        ledgerId: '',
        payload: {'name': 'Renamed', 'kind': 'expense'},
      );
  ...
  group('apply delete change', () {
    ...
    test('server 推 category:delete → 引用它的交易 category_id 置空', () async {
      ...
      final cats = await db.select(db.categories).get();
      expect(
        cats.where((c) => c.syncId == 'cat-del' || c.syncId == 'cat-del-child'),
        isEmpty,
        reason: '父分类与子分类都应被删除',
      );
      ...
      final txs = await db.select(db.transactions).get();
      expect(
        txs.firstWhere((t) => t.syncId == 'tx-cat-ref').categoryId,
        isNull,
        reason: '引用已删分类的交易 category_id 应置空，避免 A6 孤儿',
      );
      expect(
        txs
            .firstWhere((t) => t.syncId == 'tx-override-ref')
            .categorySyncIdOverride,
        isNull,
        reason: '共享账本 override 引用也应同步清空',
      );
    });
  });
  ...
  group('apply 边界', () {
    ...
    test('pull 账本币种变更重算 nativeAmount 但不登记 local_changes', () async {
      ...
      provider.pushFakeChange(
        ...
        entitySyncId: 'L1',
        ledgerId: '',
        payload: {'ledgerName': 'L', 'currency': 'USD'},
      );
      ...
      final tx = await (db.select(
        db.transactions,
      )..where((t) => t.syncId.equals('tx-currency'))).getSingle();
      expect(
        tx.nativeAmount,
        10000,
  ...
  group('legacy backfill', () {
    test('exchange_rate_override 无变更记录时补登记 upsert', () async {
      ...
      final changes = await (db.select(
        db.localChanges,
      )..where((c) => c.entityType.equals('exchange_rate_override'))).get();
      expect(changes, hasLength(1));
  ...
  group('SyncEvent stream(PR 1 解耦改造)', () {
    ...
    test('sync push 后清缓存 + emit,getStatus 从 localNewer 刷新为 inSync'
      ...
      // push 后清 _statusCache,getStatus 不读旧缓存;否则「我的」页需手动下拉才更新。
      final after = await engine.getStatus(ledgerId: ledgerId);
      expect(
        after.diff,
        SyncDiff.inSync,
        reason:
            'push 成功后 getStatus 必须刷新为 inSync;'
  ...
  group('WS realtime', () {
    test(
      'startListeningRealtime + 模拟 WS sync_change → 1s debounce 后触发 pull',
      () async {
        await db
            .into(db.ledgers)
            .insert(
...
''';
    ...
    Future<int> newLedger(String syncId) => db
        .into(db.ledgers)
        .insert(LedgersCompanion.insert(name: 'L', syncId: Value(syncId)));

    Future<int> countTx() async =>
        (await db.select(db.transactions).get()).length;

    test('P3 pullIncremental：已同步设备无新变更 → 返回0且不触发全量(防翻倍)', () async {
    ...
    test('P2 downloadAndRestoreToCurrentLedger：已同步账本不触发全量(防翻倍)', () async {
      final lid = await newLedger('ledger-1');
    ...
    test('P2 downloadAndRestoreToCurrentLedger：空账本触发全量恢复(新设备契约)', () async {
      final lid = await newLedger('ledger-1');
  ...
  group('自愈:游标越过历史 change(self-heal)', () {
    ...
    /// 被直接物理删除(未登记 delete change)。此后增量 pull 恒返回 0,
    /// 数据拉不回,需 self-heal 兜底。
    ...
    test('红测试:sync() 应检测本地缺失并自愈恢复(remoteOnly>0 → 重放)', () async {
      ...
      expect(
        ...
        hasLength(2),
        reason:
            '游标越过历史 change 时,sync() 必须自愈把云端数据拉回本地;'
    ...
    test('闸门:本地有未推送变更时不触发自愈(防未推送 delete 造成误判)', () async {
      final lid = await setupCursorOvertakesHistory();
    ...
    test('熔断:stats 口径恒偏大时,连续失败 2 次后熔断不再重试', () async {
      ...
      await engine.pullIncrementalWithHeal(ledgerId: lid);
      expect(
        engine.selfHealBroken(lid.toString()),
        isFalse,
        reason: '首次失败只计数,不熔断',
      );
    ...
    test('快照兜底:重放拉不回时(本设备变更被 server 排除)走快照恢复', () async {
      final lid = await setupCursorOvertakesHistory();
...
''';
    ...
    Future<int> setupGap(String ledgerSyncId) async {
      ...
      await (db.delete(
        db.transactions,
      )..where((t) => t.ledgerId.equals(lid))).go();
      expect((await db.select(db.transactions).get()), isEmpty);
      // 让 sync() 的 fullPush 决策命中"远端已有账本",走增量路径
    ...
    test('负向:健康设备 withHeal 返回全零 outcome,且只打 1 次 /stats', () async {
    ...
    test('负向:readLedgerStats 抛错时自愈静默吞错,不崩溃也不误 heal', () async {
      final lid = await setupCursorOvertakesHistory();
      ...
      expect(outcome.healed, 0);
      expect(outcome.gapRemaining, isFalse);
      expect(outcome.circuitBroken, isFalse, reason: '错误路径不计为自愈失败,不误触熔断');
      expect(
        await db.select(db.transactions).get(),
        isEmpty,
    ...
    test('节流:窗口内再次丢失数据也不重复自愈(拦截的是自愈动作本身)', () async {
      final lid = await setupCursorOvertakesHistory();
    ...
    test('闸门:异账本未推送变更不阻挡本账本自愈(per-ledger 闸门)', () async {
      ...
      expect(outcome.didHeal, isTrue, reason: 'per-ledger 闸门:异账本未推送不应误伤本账本');
      expect(
        outcome.healed,
      ...
      // ledger-1 恢复 2 条。
      final txs1 = await (db.select(
        db.transactions,
      )..where((t) => t.ledgerId.equals(lid1))).get();
      expect(txs1, hasLength(2));
      // ledger-2 未受影响:2 条已同步 + 1 条未推送,共 3 条。
      final txs2 = await (db.select(
        db.transactions,
      )..where((t) => t.ledgerId.equals(lid2))).get();
      expect(txs2, hasLength(3), reason: 'ledger-2 的未推送变更不应被本账本自愈误伤/误推');
    });
    ...
    test('熔断:不同账本的熔断状态相互独立(per-ledger 熔断)', () async {
      ...
      expect(
        engine.selfHealBroken(lid1.toString()),
        isFalse,
        reason: '首次失败只计数',
      );
      engine.debugClearSelfHealThrottle();
      ...
      await (db.delete(
        db.transactions,
      )..where((t) => t.ledgerId.equals(lid2))).go();
      engine.debugClearSelfHealThrottle();
      ...
      expect(
        engine.selfHealBroken(lid2.toString()),
        isFalse,
        reason: 'ledger-2 未熔断',
      );
    });
  ...
  group('syncLedgersFromServer byName 收编 — 防跨用户数据泄露(L1)', () {
    ...
    test('个人默认账本不因同名共享账本被误收编', () async {
      ...
      expect(inserted, greaterThanOrEqualTo(1), reason: '远端共享账本应作为新行被插入');
      ...
      final personal = await (db.select(
        db.ledgers,
      )..where((l) => l.id.equals(personalId))).getSingle();
      expect(
        personal.syncId,
        isNull,
        reason: '个人默认账本不应被同名共享账本收编,syncId 必须保持 null',
      );
      expect(personal.isShared, isFalse, reason: '个人账本身份不得被改成共享账本');
      expect(
        personal.myRole,
        isNot(equals('editor')),
      ...
      final shared = await (db.select(
        db.ledgers,
      )..where((l) => l.syncId.equals('shared-X'))).getSingle();
      expect(shared.isShared, isTrue);
      expect(shared.myRole, 'editor');
      expect(
        shared.id,
        isNot(equals(personalId)),
    ...
    test('远端个人默认账本仍正常收编本地同名个人账本(不误伤恢复流程)', () async {
      ...
      expect(all, hasLength(1), reason: '收编应原地更新,不应新增行(确认修复不误伤正常流程)');
    });
  });
}
