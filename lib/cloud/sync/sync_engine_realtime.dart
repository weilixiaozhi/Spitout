...
extension SyncEngineRealtime on SyncEngine {
  ...
  void startListeningRealtime() {
    ...
    _realtimeSubscription = provider.realtimeEvents.listen((event) {
      if (event.type == 'sync_change' || event.type == 'backup_restore') {
        ...
        // A 设备改外观（含支出颜色方案）/ 头像 → server 广播。这里拉一下
        // /profile/me,把 appearance 写回本地,让 B 无感同步。
  ...
  void _scheduleAutoSync({required String reason}) {
    ...
    _autoSyncDebounce = Timer(const Duration(seconds: 2), () async {
      ...
      _autoSyncing = true;
      try {
        // auto sync 为账户级同步,不依赖当前账本 —— 用户选中本地
        // 账本时云端账本也能正常同步,同时删掉了 ledgerIdResolver 反向读通道。
        logger.info('SyncEngine',
            'auto sync 触发 (reason=$reason, 账户级)');
        // 共享账本:WS 重连 / 网络恢复时,顺手对账 ledger 列表 + 共享账本
        ...
        logger.info('SyncEngine',
            'auto sync 完成 (reason=$reason): pushed=${result.pushed} '
  ...
  Future<void> _handleMemberChange(SpitoutCloudRealtimeEvent event) async {
    ...
    try {
      if (changeType == 'removed' && affectedUserId != null && affectedUserId == myUserId) {
        // 忽略「自己发起 moveToLocal 删云端」的回声:服务端删账本时会给 owner
        // 自己也广播一份 member_change.removed,若不拦截会误 purge 本地账本
        // (个人云端账本移动到本地被误删的根因)。集合由 moveToLocal 的 finally
        // 保证清空,故时序上安全——此判断只抑制自己发起的 detach 回声,
        // 不吞任何真实删除事件(集合外的 removed 照常 purge)。
        if (_pendingMoveToLocalSyncIds.contains(ledgerExternalId)) {
          logger.info('SyncEngine',
              'member_change.removed 命中 moveToLocal 忽略集合,跳过 purge ledger=$ledgerExternalId');
          return;
        }
        // 自己被踢:清本地该 ledger 数据
      ...
      // 共享账本:自己在 web 端 accept invite → server 广播 member_change.
      ...
      //   2. replayAllChanges 把 sync_changes 表所有 tx 变更重新 apply 到本地
      //      (单跑 _pull 拉不回 — 设备 cursor 已经在最新位置)
  ...
  /// 处理 Owner user-global category 变更的 fan-out。
  /// 直接增量更新本地 SharedLedgerCategories 行(写主表是
  /// Owner 操作,Editor 端只镜像)。
  Future<void> _handleSharedResourceChange(SpitoutCloudRealtimeEvent event) async {
    final ledgerExternalId = event.ledgerId;
    if (ledgerExternalId == null || ledgerExternalId.isEmpty) return;
    ...
    // Mobile serialize tag/category 时 key 是 camelCase('syncId'),
  ...
  /// 拉 server `/shared-resources` snapshot + 全量写入 SharedLedgerCategories
  /// 表(先清旧后插)。自定义图标走 sha256 cache 异步下载。
  ///
  /// 复用场景:
  ...
  /// 退出账本 / Owner 删账本 / 测试残留(user_ 前缀)数据都靠这条兜底。
  Future<void> _gcOrphanSharedLedgerRows() async {
    ...
    if (c > 0) {
      logger.info('SyncEngine',
          'GC SharedLedger* orphans: categories=$c');
    }
  }

  /// 清本地某共享账本所有数据(被踢 / Owner 删账本 / 自己退出 / 恢复 re-claim
  /// 失败后 GC1 补清)。
  ///
  /// 设计意图:清除逻辑由 [repo.purgeSharedLedger](数据层) 承载,本方法只做
  /// 编排与兜底日志。三处调用(WS 在线 / WS 离线+pull / 离线+pull 错过+GC1)
  /// 全部汇到同一入口,repo 内部按 syncId 取全部本地行(含 dup 行)幂等清除,
  /// 本地无该账本时直接早退,不会误删其它数据,也不会与 GC1 路径冲突。
  Future<void> _purgeLocalLedgerByExternalId(String ledgerExternalId) async {
    try {
      await repo.purgeSharedLedger(ledgerExternalId);
    } catch (e, st) {
      // best-effort:单轮 purge 失败不应中断整轮 GC1 / WS 处理,只记日志继续。
      logger.warning('SyncEngine',
          'purge 本地 ledger=$ledgerExternalId 失败(继续): $e', st);
    }
  }
