...
class AppCursorStore {
  ...
  /// 一次性迁移:把 cloud-sync 包内的 provider cursor 复制到
  ...
  Future<String?> _key() async {
    final auth = _provider.auth as SpitoutCloudAuthService;
...
/// 这张表。UI 据此显示 banner + 详情列表,**只读不可处置** —— Spitout Cloud
...
class SyncErrorStore {
  ...
  final SpitoutDatabase _db;

  /// 记录一条 apply 失败。同 [change.changeId] 已存在 → attempt_count++。
  ///
  /// 采用 **update-first**:先按 changeId 尝试 update,affected=0 才 insert,
  /// insert 失败再降级为 update。避免并发场景下(两个 pull 同时 select 到空、
  /// 同时 insert)第二个撞 UNIQUE 的 race。
  Future<void> record({
    required SpitoutCloudSyncChange change,
  ...
  String _encodeChange(SpitoutCloudSyncChange change) {
    return jsonEncode({
...
/// tag 的 syncId 映射,10k 条 ≈ 80-100k SELECT,iOS SQLite 主线程
/// 阻塞数十分钟(用户实测 1 万条 20 分钟)。
///
/// 改造后:pull 入口 [prime] 一次性加载 ledger/category/tag 三张表全表
/// syncId→id,apply 内查 cache,miss 才走 DB(insert 新实体后调 [putXxx]
/// 写回)。10k 条 SELECT 从 ~10万 降到 ~5(prime)+ 极少量 miss。
///
...
class LookupCache {
  ...
  Future<void> prime(SpitoutDatabase db) async {
